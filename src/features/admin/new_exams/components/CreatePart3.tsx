...
const CreatePart3: React.FC<CrPartProps> = ({ onUpdate }) => {
  const [group, setGroup] = useState<number>(
    TOEIC_PARTS.Part3.startGroupQuestionIndex,
  ...
  return (
    <>
      ...
      >
        {part3Group.map((_, internalGroupIndex) => {
          const actualGroupIndex =
            internalGroupIndex + TOEIC_PARTS.Part3.startGroupQuestionIndex;

          const startQuestionNumber =
            TOEIC_PARTS.Part3.start +
            internalGroupIndex * TOEIC_PARTS.Part3.questionPerGroup;
          const endQuestionNumber =
            startQuestionNumber + TOEIC_PARTS.Part3.questionPerGroup - 1;
...
const Part3Form: React.FC<Part1FormProps> = ({ groupIndex, onClose }) => {
  const form = useFormContext<SaveToeicTestRequest>();
          ...
          return (
            <Box key={questionIndex} mb={1}>
              <Stack direction="row" spacing={0.5} sx={{ my: 1 }}>
                ...
                >
                  {TOEIC_PARTS.Part3.start +
                    (groupIndex - TOEIC_PARTS.Part3.startGroupQuestionIndex) *
                      TOEIC_PARTS.Part3.questionPerGroup +
                    questionIndex}
                </Box>
                <TextField
                  label="Question"
