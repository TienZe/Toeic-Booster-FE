...
export default function TOEICChatbot() {
  ...
  const [highlightedIndex, setHighlightedIndex] = useState(0); // current index of question that is focus on suggestion modal
  ...
  const createNewChatMutation = useMutation({
    mutationFn: async () => {
      if (focusQuestionNumber) {
        return await createChatHistory({
          attemptId: attemptId!,
  ...
  const sendMessage = async (messageText: string) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: "model",
        textResponse: "Xin lỗi, có lỗi xảy ra. Vui lòng thử lại sau. 😔",
      };
            ...
            >
              ...
              <Box
                ...
                sx={{
                  p: 1,
                  background:
                    "linear-gradient(135deg, #203A90 0%, #4A5FA8 100%)",
                  color: "white",
                  display: "flex",
                  alignItems: "center",
